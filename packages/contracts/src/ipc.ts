...
import type {
  ...
  ProviderGetInstallMethodsResult,
  InstallMethodId,
...
export interface DesktopBridge {
  ...
  /** Electron only: opens Chromium DevTools in a detached window for the main BrowserWindow. */
  openDetachedDevTools: () => Promise<void>;
...
export interface LocalApi {
  ...
  provider: {
    getInstallMethods: () => Promise<ProviderGetInstallMethodsResult>;
    install: (
  ...
  claudeCode: {
    install: (platform?: "linux" | "darwin" | "win32") => Promise<ClaudeCodeInstallResult>;
  };
  openCode: {
    install: (platform?: "linux" | "darwin" | "win32") => Promise<OpenCodeInstallResult>;
  };
}
