...
export const makeOrchestrationIntegrationHarness = (
  ...
  Effect.gen(function* () {
    ...
    const textGenerationLayer = Layer.succeed(TextGeneration, {
      generateBranchName: () => Effect.succeed({ branch: "更新" }),
      generateThreadTitle: () => Effect.succeed({ title: "新项目" }),
    ...
    const checkpointReactorLayer = CheckpointReactorLive.pipe(
      ...
      Layer.provideMerge(
        Layer.succeed(GitStatusBroadcaster, {
          getStatus: () => Effect.die("getStatus 不应在此测试中调用"),
          refreshLocalStatus: () =>
            Effect.succeed({
              isRepo: true,
          ...
          refreshStatus: () => Effect.die("refreshStatus 不应在此测试中调用"),
