...
const threadId = ThreadId.make("thread-ssh-claude");
const projectId = ProjectId.make("project-ssh-claude");
...
const makeSshExecution = (execCalls: Array<string>): WorkspaceExecution => ({
  ...
  spawnInteractive: () => Effect.die("unused in resolveClaudeSpawn test"),
  exec: ({ command }) =>
    Effect.sync(() => {
...
SshResolveLayer("resolveClaudeSpawnForThread", (it) => {
  it.effect("resolves ssh spawn with probed remote binary path", () =>
    Effect.gen(function* () {
      sshExecCalls.length = 0;
      const spawn = yield* resolveClaudeSpawnForThread(threadId, "claude");

      assert.equal(spawn.kind, "ssh");
      if (spawn.kind !== "ssh") {
        return;
      }
      assert.equal(spawn.binaryPath, "/usr/bin/claude");
      assert.equal(sshExecCalls.length, 1);
      assert.include(sshExecCalls[0] ?? "", "claude");
    }),
  );
});
...
LocalResolveLayer("resolveClaudeSpawnForThread local", (it) => {
