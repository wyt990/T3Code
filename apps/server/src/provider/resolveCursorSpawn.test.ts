...
const threadId = ThreadId.make("thread-ssh-cursor");
const projectId = ProjectId.make("project-ssh-cursor");
...
const makeSshExecution = (execCalls: Array<string>): WorkspaceExecution => ({
  ...
  spawnInteractive: () => Effect.die("unused in resolveCursorSpawn test"),
  exec: ({ command }) =>
    Effect.sync(() => {
      execCalls.push(command);
      return {
        stdout: "/usr/bin/agent\n",
        stderr: "",
        exitCode: 0,
      };
    }),
...
SshResolveLayer("resolveCursorSpawnForThread", (it) => {
  it.effect("resolves ssh spawn with probed remote binary path", () =>
    Effect.gen(function* () {
      sshExecCalls.length = 0;
      const spawn = yield* resolveCursorSpawnForThread(threadId, "agent");

      assert.equal(spawn.kind, "ssh");
      if (spawn.kind !== "ssh") {
        return;
      }
      assert.equal(spawn.binaryPath, "/usr/bin/agent");
      assert.equal(sshExecCalls.length, 1);
      assert.include(sshExecCalls[0] ?? "", "agent");
    }),
  );
});
...
LocalResolveLayer("resolveCursorSpawnForThread local", (it) => {
