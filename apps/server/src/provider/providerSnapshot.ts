...
function isWindowsCommandNotFoundError(message: string): boolean {
  // Windows system error messages (Chinese and English)
  // The garbled text is from Windows Chinese error message encoded incorrectly.
  // We intentionally detect both raw mojibake bytes and replacement-char sequences.
  const windowsNotFoundPatterns = [
    /not recognized as an internal or external command/i,
    /command not found/i,
    // Match garbled Windows Chinese errors (GBK/CP936 encoded incorrectly as UTF-8).
    /[\x80-\xff]{3,}/, // Any sequence of non-ASCII bytes (garbled encoding)
    /\uFFFD{2,}/, // UTF-8 replacement characters from decode failures
  ];
  return windowsNotFoundPatterns.some((pattern) => pattern.test(message));
}
...
export function detailFromResult(
  ...
  if (stderr) {
    // Filter out Windows "command not found" errors with garbled encoding
    if (isWindowsCommandNotFoundError(stderr)) {
  ...
  if (stdout) {
    // Filter out Windows "command not found" errors with garbled encoding
    if (isWindowsCommandNotFoundError(stdout)) {
      return "Command not found.";
    }
    return stdout;
  }
