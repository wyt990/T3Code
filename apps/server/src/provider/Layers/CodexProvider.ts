...
const makePendingCodexProvider = (codexSettings: CodexSettings): ServerProvider => {
  ...
  return buildServerProvider({
    ...
    probe: {
      ...
      status: "warning",
      auth: { status: "unknown" },
      message: "本次会话尚未检查 Codex 服务提供商状态。",
    },
  });
};
...
function accountProbeStatus(account: CodexAppServerProviderSnapshot["account"]): {
  ...
  if (account.requiresOpenaiAuth) {
    return {
      ...
      message: "Codex CLI 未认证。请运行 `codex login` 并重试。",
    };
  }
  ...
  if (Result.isFailure(probeResult)) {
    ...
    return buildServerProvider({
      ...
      probe: {
        ...
        auth: { status: "unknown" },
        message: installed
          ? `Codex app-server 提供商检查失败: ${error.message}.`
          : "Codex CLI (`codex`) 未安装或未在 PATH 中。",
      },
    });
  }
  ...
  if (Option.isNone(probeResult.success)) {
    return buildServerProvider({
      ...
      probe: {
        ...
        status: "error",
        auth: { status: "unknown" },
        message: "检查 Codex app-server 提供商状态超时。",
      },
    });
  }
