...
function sessionErrorMessage(error: unknown): string {
  ...
  if (message.includes("eager_input_streaming")) {
    return "当前 OpenCode 上游模型不兼容工具参数（eager_input_streaming）。请切换到兼容模型（例如 opencode/gpt-5-nano）或在模型选项中改用不依赖工具的 Agent 后重试。";
  }
  return message;
}
