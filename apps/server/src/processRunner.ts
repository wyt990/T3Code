...
function decodeBuffer(chunk: Buffer): string {
  // On Windows, decode using GBK (CP936) for proper Chinese character support
  // PowerShell/cmd output uses the system code page which is GBK on Chinese Windows
  return process.platform === "win32" ? iconv.decode(chunk, "gbk") : chunk.toString("utf-8");
}
...
function appendChunkWithinLimit(
  ...
  if (chunk.length <= remaining) {
    return {
      next: `${target}${decodeBuffer(chunk)}`,
      nextBytes: currentBytes + chunk.length,
      truncated: false,
    };
  }
  return {
    next: `${target}${decodeBuffer(chunk.subarray(0, remaining))}`,
...
export async function runProcess(
  ...
  return new Promise<ProcessRunResult>((resolve, reject) => {
    // On Windows, force PowerShell/cmd to use UTF-8 output encoding
    const win32Env =
      process.platform === "win32"
        ? {
            // PowerShell output encoding
            PS_OUTPUT_ENCODING: "utf8",
            // .NET runtime encoding
    ...
    const child = spawn(command, args, {
      ...
      env: {
        ...process.env,
        ...options.env,
        // Force UTF-8 encoding for child processes (fixes Chinese garbled text on Windows)
    ...
    const appendOutput = (stream: "stdout" | "stderr", chunk: Buffer | string): Error | null => {
      ...
      // Decode buffer using appropriate encoding for the platform
      const text = decodeBuffer(chunkBuffer);
