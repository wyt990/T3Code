...
const make = Effect.gen(function* () {
  ...
  const getProjectionSnapshotQuery = Effect.fn("lazy.ProjectionSnapshotQuery")(function* () {
    return yield* ProjectionSnapshotQuery;
  });

  const getTurnDiff = Effect.fn("getTurnDiff")(function* (
    ...
    const projectionSnapshotQuery = yield* getProjectionSnapshotQuery();
  ...
  const getFullThreadDiff = (input: OrchestrationGetFullThreadDiffInput) =>
    getTurnDiff({
      threadId: input.threadId,
      fromTurnCount: 0,
      toTurnCount: input.toTurnCount,
