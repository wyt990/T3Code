...
export const MessagesTimeline = memo(function MessagesTimeline({
  ...
  const renderItem = useCallback(
    ({ item }: { item: MessagesTimelineRow }) => (
      <div className="w-full min-w-0 overflow-x-hidden" data-timeline-root="true">
              ...
              {row.showCompletionDivider && (
                <div className="my-3 flex items-center gap-3">
                  ...
                  <span className="rounded-full border border-border bg-background px-2.5 py-1 text-[10px] uppercase tracking-[0.14em] text-muted-foreground/80">
                    {ctx.completionSummary ? `响应 • ${ctx.completionSummary}` : "响应"}
                  </span>
                  <span className="h-px flex-1 bg-border" />
                </div>
              )}
        ...
        <div className="py-0.5 pl-1.5">
          <div className="flex items-center gap-2 pt-1 text-[11px] text-muted-foreground/70">
            ...
            <span>
              {row.createdAt ? (
                <>
                  工作中 for <WorkingTimer createdAt={row.createdAt} />
                </>
              ) : (
                "正在工作中..."
              )}
...
const WorkGroupSection = memo(function WorkGroupSection({
  ...
  const groupLabel = onlyToolEntries ? "工具调用" : "工作日志";
      ...
      {showHeader && (
        <div className="mb-1.5 flex items-center justify-between gap-2 px-0.5">
          ...
          {hasOverflow && (
            ...
            >
              {isExpanded ? "显示更少" : `显示 ${hiddenCount} 更多`}
            </button>
          )}
        </div>
      )}
    ...
    <div className="mt-2 rounded-lg border border-border/80 bg-card/45 p-2.5">
      <div className="mb-1.5 flex items-center justify-between gap-2">
        <p className="text-[10px] uppercase tracking-[0.12em] text-muted-foreground/65">
          <span>更改的文件 ({changedFileCountLabel})</span>
        ...
        <div className="flex items-center gap-1.5">
          ...
          >
            {allDirectoriesExpanded ? "折叠所有" : "展开所有"}
          </Button>
          <Button
            type="button"
            size="xs"
