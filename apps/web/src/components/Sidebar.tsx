...
function projectGroupingModeDescription(mode: SidebarProjectGroupingMode): string {
  switch (mode) {
    case "repository":
      return "来自同一仓库的项目共享一个侧边栏行。";
    case "repository_path":
      return "仅当仓库和仓库相对路径都匹配时，项目才会分组。";
    case "separate":
      return "每个项目路径都有独立的侧边栏行。";
  }
}
...
const SidebarThreadRow = memo(function SidebarThreadRow(props: SidebarThreadRowProps) {
  ...
  const threadEnvironmentLabel = isRemoteThread
    ? (remoteEnvLabel ?? remoteEnvSavedLabel ?? "远程")
    : null;
...
const SidebarProjectItem = memo(function SidebarProjectItem(props: SidebarProjectItemProps) {
                ...
                void (async () => {
                  ...
                  const confirmed = await api.dialogs.confirm(
                    latestProjectThreads.length > 0
                      ? [
                          `移除项目 "${member.name}" 并删除其 ${latestProjectThreads.length} 个会话？`,
                          `路径: ${member.cwd}`,
  ...
  const handleProjectButtonContextMenu = useCallback(
    (event: React.MouseEvent<HTMLButtonElement>) => {
      ...
      void (async () => {
        ...
        const makeLeaf = (
          action: "new-thread" | "rename" | "grouping" | "copy-path" | "delete",
          member: SidebarProjectGroupMember,
          options?: {
            destructive?: boolean;
            disabled?: boolean;
        ...
        const clicked = await api.contextMenu.show(
          [
            buildTargetedItem("new-thread", "新建会话"),
            buildTargetedItem("rename", "重命名项目"),
            buildTargetedItem("grouping", "项目分组…"),
            buildTargetedItem("copy-path", "复制项目路径"),
            buildTargetedItem("delete", "移除项目", {
              destructive: true,
            }),
          ],
          {
  ...
  const handleThreadContextMenu = useCallback(
    async (threadRef: ScopedThreadRef, position: { x: number; y: number }) => {
      ...
      if (appSettingsConfirmThreadDelete) {
        const confirmed = await api.dialogs.confirm(
          [`删除会话 "${thread.title}"？`, "这将永久清除此会话的对话历史。"].join("\n"),
        );
        if (!confirmed) {
          return;
        }
    ...
    <>
      <div className="group/project-header relative">
          ...
          <Tooltip>
            <TooltipTrigger
              render={
                <span
                  aria-label={
                    project.environmentPresence === "remote-only" ? "远程项目" : "在多个环境中可用"
                  }
        ...
        <Tooltip>
          <TooltipTrigger
            render={
              <div className="pointer-events-none absolute top-1 right-1.5 opacity-0 transition-opacity duration-150 group-hover/project-header:pointer-events-auto group-hover/project-header:opacity-100 group-focus-within/project-header:pointer-events-auto group-focus-within/project-header:opacity-100">
                <button
                  type="button"
                  aria-label={`在 ${project.displayName} 中创建新会话`}
          ...
          />
          <TooltipPopup side="top">
            {newThreadShortcutLabel ? `新会话 (${newThreadShortcutLabel})` : "新会话"}
          </TooltipPopup>
        </Tooltip>
      </div>
      ...
      >
        <DialogPopup className="max-w-lg">
          <DialogHeader>
            ...
            <DialogDescription>
              {projectRenameTarget ? `更新 ${projectRenameTarget.cwd} 的标题。` : "更新项目标题。"}
            </DialogDescription>
          </DialogHeader>
          <DialogPanel className="space-y-4">
            <div className="grid gap-1.5">
              <span className="text-xs font-medium text-foreground">项目标题</span>
              <Input
                aria-label="项目标题"
      ...
      >
        <DialogPopup className="max-w-lg">
          <DialogHeader>
            ...
            <DialogDescription>
              {projectGroupingTarget
                ? `选择 ${projectGroupingTarget.cwd} 在侧边栏中的分组方式。`
                : "选择此项目在侧边栏中的分组方式。"}
            </DialogDescription>
          </DialogHeader>
          <DialogPanel className="space-y-4">
                ...
                <SelectTrigger className="w-full" aria-label="项目分组规则">
                  <SelectValue>
                    {projectGroupingSelection === "inherit"
                      ? `使用全局默认 (${PROJECT_GROUPING_MODE_LABELS[projectGroupingSettings.sidebarProjectGroupingMode]})`
  ...
  return (
    <Menu>
      <Tooltip>
        ...
        <TooltipPopup side="right">排序项目</TooltipPopup>
      </Tooltip>
  ...
  return (
    <SidebarContent className="gap-0">
      ...
      {showArm64IntelBuildWarning && arm64IntelBuildWarningDescription ? (
        <SidebarGroup className="px-2 pt-2 pb-0">
          <Alert variant="warning" className="rounded-2xl border-warning/40 bg-warning/8">
            <TriangleAlertIcon />
            <AlertTitle>Apple Silicon 上的 Intel 版本</AlertTitle>
      ...
      <SidebarGroup className="px-2 py-2">
        ...
        {projectsLength === 0 && (
          <div className="px-2 pt-4 text-center text-xs text-muted-foreground/60">暂无项目</div>
        )}
      </SidebarGroup>
    </SidebarContent>
  );
